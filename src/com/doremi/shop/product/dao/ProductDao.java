...
//		System.out.println(list);
	...
	public List<Product> findByPageName(String searchText, int begin, int limit) {
		String hql = "from Product where pname like ?";
		String s = "%"+searchText+"%";
		
		List<Product> list = this.getHibernateTemplate().execute(new PageHibernateCallback<Product>(hql, new Object[]{s}, begin, limit));
		if(list!=null&&list.size()>0){
			return list;
		}
		return null;
	...
	public List<Product> findByKey(String key, int begin, int limit) {
		String hql = "from Product where pname like ?";
		String s = "%"+key+"%";
		List<Product> list = this.getHibernateTemplate().execute(
				new PageHibernateCallback<Product>(hql,new Object[]{s},
						begin, limit));
		if (list != null && list.size() > 0) {
			return list;
		}
